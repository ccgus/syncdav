...
@class FMTCPConnection, FMTCPWriter, FMIPAddress;


/** Abstract superclass for data streams, used by TCPConnection. */
@interface FMTCPStream : NSObject <NSStreamDelegate>
{
    FMTCPConnection *_conn;
    NSStream *_stream;
    BOOL _shouldClose;
}
...
@property (readonly) FMIPAddress *peerAddress;
...
@interface FMTCPReader : FMTCPStream {
    void (^_canReadBlock)(FMTCPReader *);
}
...
- (NSString*)stringFromReadData;//:(NSUInteger)maxData;

@end



@interface FMTCPStream (Protected)
