...
@class FMTCPConnection, FMIPAddress;
@protocol FMTCPListenerDelegate;
...
@interface FMTCPListener : FMTCPEndpoint <NSNetServiceDelegate>
{
    @private
    uint16_t _port;
...
@property (assign) id<FMTCPListenerDelegate> delegate;
...
@protocol FMTCPListenerDelegate <NSObject>
...
- (void)listener: (FMTCPListener*)listener didAcceptConnection: (FMTCPConnection*)connection;

@optional
/** Called after the listener successfully opens. */
- (void)listenerDidOpen: (FMTCPListener*)listener;
/** Called if the listener fails to open due to an error. */
- (void)listener: (FMTCPListener*)listener failedToOpen: (NSError*)error;
/** Called after the listener closes. */
- (void)listenerDidClose: (FMTCPListener*)listener;
...
- (BOOL)listener: (FMTCPListener*)listener shouldAcceptConnectionFrom: (FMIPAddress*)address;
@end
