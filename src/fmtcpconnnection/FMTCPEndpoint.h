...
/** This defines the SSL identity to be used by this endpoint.
    ...
    validation chain. */
#define kFMTCPPropertySSLCertificates  ((NSString*)kCFStreamSSLCertificates)
...
#define kFMTCPPropertySSLAllowsAnyRoot ((NSString*)kCFStreamSSLAllowsAnyRoot)
...
#define kFMTCPPropertySSLPeerName      ((NSString*)kCFStreamSSLPeerName)
...
typedef enum {
	kFMTCPNeverAuthenticate,			/* skip client authentication */
	kFMTCPAlwaysAuthenticate,         /* require it */
	kFMTCPTryAuthenticate             /* try to authenticate, but not error if client has no cert */
} FMTCPAuthenticate; // these MUST have same values as SSLAuthenticate enum in SecureTransport.h!


/** Abstract base class of TCPConnection and TCPListener.
    Mostly just manages the SSL properties. */
@interface FMTCPEndpoint : NSObject
{
